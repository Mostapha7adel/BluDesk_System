...
import {
  ...
  Stack, MenuItem, TextField, Avatar, Chip, Alert, IconButton
} from '@mui/material';
import { Plus, DollarSign, CheckCircle, XCircle, Loader, Users } from 'lucide-react';
import { Container as Grid, Item as GridItem } from '../../components/ui/Grid';
import PageHeader from '../../components/ui/PageHeader';
import DataTable from '../../components/tables/DataTable';
import StatusBadge from '../../components/ui/StatusBadge';
import { formatCurrency, formatDate } from '../../utils/format';
import { useSalaries, useCreateSalary, useApproveSalary, usePaySalary, useCancelSalary, usePendingSalaries, useEmployees } from '../../hooks/api';
...
export default function Salaries() {
  ...
  const columns = [
    ...
    { key: 'amount', label: t('salaries.amount'), render: (row) => formatCurrency(parseFloat(row.amount)) },
  ...
  return (
    <Box>
      <PageHeader
        ...
        actionLabel={t('salaries.addSalary')}
...
function SalaryDialog({ open, onClose, t, employees, onSubmit, error }) {
  const [form, setForm] = useState({ employeeId: '', amount: '', month: String(currentMonth), year: String(currentYear), notes: '' });
  const handleChange = (f) => (e) => setForm({ ...form, [f]: e.target.value });

  useEffect(() => {
    if (!open) setForm({ employeeId: '', amount: '', month: String(currentMonth), year: String(currentYear), notes: '' });
  }, [open]);
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Stack spacing={2.5} sx={{ mt: 1 }}>
          ...
          <TextField label={t('employees.name')} select size="small" fullWidth value={form.employeeId} onChange={handleChange('employeeId')}>
            {employees.map((emp) => (
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <TextField label={t('salaries.amount')} type="number" size="small" fullWidth value={form.amount} onChange={handleChange('amount')} />
